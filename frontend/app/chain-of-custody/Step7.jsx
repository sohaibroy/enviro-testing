...
export default function Step7({ onNext, onBack }) {
  ...
  return (
 <div className="w-full max-w-7xl bg-white shadow-xl p-4 sm:p-6 md:p-8 lg:p-10 rounded-xl space-y-6 flex flex-col"
     style={{ 
       minHeight: ['80vh', '80vh', '90vh'], 
       overflowY: 'auto' 
     }}>
      <h2 className="text-xl sm:text-2xl md:text-3xl font-semibold text-center text-gray-800">Step 7: Equipment Rental</h2>
      ...
      <div className="flex flex-col lg:flex-row gap-4 sm:gap-5 md:gap-6 flex-1">
        ...
        <div className="w-full lg:w-1/3 bg-white shadow-sm sm:shadow-md rounded-lg p-3 sm:p-4 border h-fit">
          <h3 className="text-base sm:text-lg md:text-lg font-bold text-gray-800 mb-2 sm:mb-3 md:mb-4 border-b pb-1 sm:pb-2">Equipment Categories</h3>
          {loadingTypes ? (
            <p className="text-gray-500">Loading categories...</p>
          ) : errorTypes ? (
            <ErrorMessage message={errorTypes} />
          ) : (
            <EquipmentCategories onSelectCategory={handleSelectCategory} equipmentTypes={equipmentTypes} />
          )}
        </div>
