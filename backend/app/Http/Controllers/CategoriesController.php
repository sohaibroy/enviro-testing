...
public function destroy($category_id): JsonResponse
    {
        if (!is_numeric($category_id) || (int)$category_id < 1) {
            return response()->json(['message' => 'Please enter a valid category id'], 400);
        }

        $exists = DB::table('categories')->where('category_id', (int)$category_id)->exists();
        if (!$exists) {
            return response()->json(['message' => 'Not found'], 404);
        }

        try {
            //FK constraints, catch it and return 409.
            DB::table('categories')->where('category_id', (int)$category_id)->delete();

            return response()->json(['message' => 'Deleted'], 200);
        } catch (\Throwable $e) {
            // \Log::error('Delete category failed', ['e' => $e->getMessage()]);
            return response()->json(['message' => 'Cannot delete this category right now'], 409);
        }
    }
}
